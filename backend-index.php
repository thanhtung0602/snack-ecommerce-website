...
function php_edit_info_db(){
    session_start();
    $conn = connect(); // Khởi tạo kết nối để dùng cho validate_input_sql
    ...
    if($ten != "")  { $n[] = 'ten'; $data[] = validate_input_sql($conn, $ten); }
    if($mk != "")   { $n[] = 'matkhau'; $data[] = validate_input_sql($conn, $mk); } 
    if($dc != "")   { $n[] = 'diachi'; $data[] = validate_input_sql($conn, $dc); } 
    if($sdt != "")  { $n[] = 'sodt'; $data[] = validate_input_sql($conn, $sdt); }
    if($email != "") { $n[] = 'email'; $data[] = validate_input_sql($conn, $email); }
...
function cart_list(){
    ...
    if(isset($_SESSION[$cart_name])){
        $arr = $_SESSION[$cart_name];
        array_shift($arr); 
        $arr = array_unique($arr);
        
        if(count($arr) == 0){
            echo "<h4>Giỏ hàng trống</h4><i>Ây da, chọn món gì đó đi bạn ơi :)</i>";
            return;
        }
        ...
        while ($row = mysqli_fetch_assoc($result)) { ?>
            <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #ddd; padding: 10px 0;">
                <div style="display: flex; align-items: center;">
                    <img src="<?php echo $row['anhchinh'] ?>" style="width: 60px; height: 60px; margin-right: 15px; border-radius: 5px;">
                    <div>
                        <h4 style="margin: 0;"><b><?php echo $row['tensp'] ?></b></h4>
                        <span style="color: red; font-weight: bold;"><?php echo number_format($row['gia']) ?> VNĐ</span>
                    </div>
                </div>
